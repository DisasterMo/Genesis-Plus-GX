...
try:
    ...
    subprocess.run(['java', '-jar', 'crowdin-cli.jar', 'upload', 'translations'])
    ...
    crowdin_config = re.sub(r'"dest": "/.*?/%original_file_name%",',
                            f'"dest": "/_core_name_/%original_file_name%",'
                            , crowdin_config, 1)
    with open('crowdin.yaml', 'w') as crowdin_config_file:
        crowdin_config_file.write(crowdin_config)

    with open('.github/workflows/crowdin_prep.yml', 'r') as crowdin_config_file:
        crowdin_config = crowdin_config_file.read()
    crowdin_config = re.sub(r'python3 intl/initial_upload\.py', 'python3 intl/crowdin_source_upload.py',
                            crowdin_config, 1)
    with open('.github/workflows/crowdin_prep.yml', 'w') as crowdin_config_file:
        crowdin_config_file.write(crowdin_config)
